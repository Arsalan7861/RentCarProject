...
public class MainODataController : ODataController
{
    ...
    [HttpGet("users")]
    public IQueryable<UserDto> Users(ISender sender, CancellationToken cancellationToken)
        => sender.Send(new UserGetAllQuery(), cancellationToken).Result;

    [HttpGet("categories")]
    public IQueryable<CategoryDto> Categories(ISender sender, CancellationToken cancellationToken)
        => sender.Send(new CategoryGetAllQuery(), cancellationToken).Result;

    [HttpGet("protection-packages")]
    public IQueryable<ProtectionPackageDto> ProtectionPackages(ISender sender, CancellationToken cancellationToken)
        => sender.Send(new ProtectionPackageGetAllQuery(), cancellationToken).Result;

    [HttpGet("extras")]
    public IQueryable<ExtraDto> Extras(ISender sender, CancellationToken cancellationToken)
        => sender.Send(new ExtraGetAllQuery(), cancellationToken).Result;

    [HttpGet("vehicles")]
    public IQueryable<VehicleDto> Vehicles(ISender sender, CancellationToken cancellationToken)
        => sender.Send(new VehicleGetAllQuery(), cancellationToken).Result;
    [HttpGet("customers")]
    public IQueryable<CustomerDto> Customers(ISender sender, CancellationToken cancellationToken)
        => sender.Send(new CustomerGetAllQuery(), cancellationToken).Result;
    [HttpGet("reservations")]
    public IQueryable<ReservationDto> Reservations(ISender sender, CancellationToken cancellationToken)
    {
        var res = sender.Send(new ReservationGetAllQuery(), cancellationToken).Result;
        return res;
    }
}
