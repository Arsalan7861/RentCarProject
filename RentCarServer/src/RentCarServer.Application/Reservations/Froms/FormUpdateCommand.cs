...
internal sealed class FormUpdateCommandHandler(
    ...
    ) : IRequestHandler<FormUpdateCommand, Result<string>>
{
    public async Task<Result<string>> Handle(FormUpdateCommand request, CancellationToken cancellationToken)
    {
        var reservation = await reservationRepository.FirstOrDefaultAsync(r => r.Id == request.ReservationId, cancellationToken);
        ...
        List<Supply> supplies = request.Supplies?.Select(s => new Supply(s)).ToList() ?? [];
        List<ImageUrl> imageUrls = request.ImageFiles.Select(i =>
        {
            var fileName = FileService.FileSaveToServer(i, "wwwroot/forms/");
        ...
        else
        {
            ...
            message = "Araç müşteriden geri alındı.";
            reservation.SetReservationStatus(Status.Completed);
            ReservationHistory history = new(
                "Araç Geri Alındı.",
                "Araç müşteriden geri alındı.",
                DateTimeOffset.Now
                );
            reservation.SetHistory(history);
        }
